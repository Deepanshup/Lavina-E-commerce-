	...
	static function sync_customers()
	{
		...
		else
		{
			...
			if ( $user_query->get_results()) {
				$groupXml="<?xml version='1.0' encoding='UTF-8'?><group><user username='".$username."' password='".$password."' grp_name='".$group_name."' />";
					
				 $cnt = 0;
				 foreach ( $user_query->get_results() as $user ) {
					 
					$number = get_user_meta($user->ID, 'billing_phone', true);
					//xml format for group
					$groupXml.="
						<members>
								<member name='".$user->display_name."' number='".$number."'><member-data field='address'>Delhi</member-data>
								</member>
						</members>
						";
				...
				$groupXml.="</group>";
				$resp = SmsAlertcURLOTP::create_contact($groupXml);
