					 ...
					 foreach($embk_booking_statuses as $ks => $vs)
					 {
						...
						<div id="accordion_<?php echo $ks; ?>" class="cvt-accordion-body-content">
							<table class="form-table">
								...
								<textarea name="smsalert_embk_message[embk_admin_sms_body_<?php echo $vs; ?>]" id="smsalert_message[admin_sms_body_<?php echo $vs; ?>]" <?php echo((smsalert_get_option( 'embk_admin_notification_'.$vs, 'smsalert_embk_general', 'on')=='on')?'' : "readonly='readonly'"); ?>><?php 
						  echo smsalert_get_option('embk_admin_sms_body_'.$vs, 'smsalert_embk_message', defined('SmsAlertMessages::DEFAULT_EMBK_ADMIN_SMS_'.str_replace('-', '_', strtoupper($vs))) ? constant('SmsAlertMessages::DEFAULT_EMBK_ADMIN_SMS_'.str_replace('-', '_', strtoupper($vs))) : SmsAlertMessages::DEFAULT_EMBK_ADMIN_SMS_STATUS_CHANGED); 
