...
/**
   ...
   {	
	   ...
	   $embk_booking_statuses 	= is_plugin_active('events-manager/events-manager.php') ? SmsAlertEMBooking::em_booking_statuses() : array();
	...
	public static function get_settings() {
		...
		<div class="SMSAlert_box SMSAlert_settings_box">
		 ...
		 <div>
			...
			<!--/--Woocommerce Booking Admin templates-->
			<?php }?>	
			<!--/-Woocommerce Booking tabs--><!--/-Woocommerce Booking tabs-->
			<!-- ============================================================================== -->
			<!--EM Booking tabs-->
			<?php if($hasEMBookings){?>
			<?php $embk_booking_statuses = SmsAlertEMBooking::em_booking_statuses();
			?>
			<!--EM Booking Customer templates-->
			
					<div class="SMSAlert_nav_box SMSAlert_nav_embkcsttemplates_box embkcsttemplates">
						
						<?php 
						$params=array('embk_booking_statuses'=>$embk_booking_statuses);
							echo get_smsalert_template('views/embooking_customer_template.php',$params);
						?>
					</div>
			<!--/--EM Booking Customer templates-->
			<!--Em Booking Admin templates-->
			
					<div class="SMSAlert_nav_box SMSAlert_nav_embkadmintemplates_box embkadmintemplates">
						<?php 
							$params=array('embk_booking_statuses'=>$embk_booking_statuses);
							echo get_smsalert_template('views/embooking_admin_template.php',$params);
						?>
					</div>
			<!--/--EM Booking Admin templates-->
			<?php }?>	
					...
					?>
						<tr valign="top">
							...
							<td>
								...
								{
								?>
									<input type="button" id="smsalert_sync_btn" onclick="doSASyncNow(this)" class="button button-primary" value="Sync Now" disabled>
								<?php		
								}
							  ?>
								<span class="tooltip" data-title="Sync users to a Group in smsalert.co.in"><span class="dashicons dashicons-info"></span></span>						  
								<span id="sync_status" style="opacity:0">0 contacts synced</span>
			...
			function insertAtCaret(textFeildValue, txtbox_id) {
				...
				function toggleDisabled(obj) {
					...
					if(jQuery('.SMSAlert_box input[type="checkbox"]').eq(t).is(":checked") === !1)
					{
						...
						if(jQuery('#group_auto_sync').prop("disabled"))
						{
							jQuery('#smsalert_sync_btn').attr("disabled",true);
						}
					}
					else
					{
						...
						if(!jQuery('#group_auto_sync').prop("disabled"))
						{
							jQuery('#smsalert_sync_btn').removeAttr("disabled");
						}
					}
