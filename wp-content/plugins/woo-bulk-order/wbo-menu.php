...
if ( ! class_exists( 'WooExtendMenu' ) ) {
	class WooExtendMenu {
		...
		public function wbo_get_free_data() {

			$arrData = array();

			$arrSetting = get_option('wbo_settings');

			$arrData['sort_by'] = $arrSetting['sort_by'];
			$arrData['sort_order'] = $arrSetting['sort_order'];
			$arrData['i' . 's'] = false;
			$arrData['ids'] = $arrSetting['ids'];
			return $arrData;
		}

		public function wbo_may_be_save_data() {

			if(isset($_POST) && !empty($_POST)) {

				global $wpdb;
				$arrData['sort_by'] = isset($_POST['rdo_sort_by']) && !empty($_POST['rdo_sort_by'])?sanitize_text_field($_POST['rdo_sort_by']):'';
				$arrData['sort_order'] = isset($_POST['rdo_sort_order']) && !empty($_POST['rdo_sort_order'])?sanitize_text_field($_POST['rdo_sort_order']):'';
				$arrData['ids'] = isset($_POST['txtProductIds']) && !empty($_POST['txtProductIds'])?sanitize_text_field($_POST['txtProductIds']):'';

				$short_code = "[wbo_woo_bulk_order " . 
								(isset($arrData['sort_by']) && !empty($arrData['sort_by'])?' orderby=' . $arrData['sort_by']:'') .
								(isset($arrData['sort_order']) && !empty($arrData['sort_order'])?' order=' . $arrData['sort_order']:'') .
								(isset($arrData['ids']) && !empty($arrData['ids'])?' id=' . $arrData['ids']:'') .
								"]";
				update_option( 'wbo_settings', $arrData);
				$wpdb->query("UPDATE {$wpdb->prefix}posts SET post_content = '" . $short_code . "' WHERE id='" . get_option('wbo_order_now_page') . "'");
			}
		}
	}

	$wbo_menu = new WooExtendMenu();
}