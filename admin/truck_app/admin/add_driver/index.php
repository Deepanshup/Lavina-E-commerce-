...
try 
{
        ...
        if(isset($_POST["register"])){
            ...
            $sql="select * from driver where name='$name' and truck='$truck' and manager_no='$manager_no' ";
            $result = mysqli_query($conn,$sql);
            if(mysqli_num_rows($result) > 0)
            {
                while($row=mysqli_fetch_row($result)){
                    $one=$row[3];
                    $messages= "Driver Already Exist.Want to update Driver??";
                }
            }
            else
            {
                $sql = "INSERT INTO `driver`(`name`,`truck`, `manager_no`) VALUES ('$name','$truck','$manager_no')";
                if ($conn->query($sql)) {
                ...
                <form class="contact-form" method="POST" action="">
                    <div class="row">
                        <div class="input-group">
                            ...
                            <input type="text" name="name"  required autofocus placeholder="Name"  class="input-group__input form-control is-invalid"/>
                                <span class="invalid-feedback col-md-12" role="alert">
                                    ...
                                    <a style="text-decoration:none" href="./update_driver.php?id=<?php echo $one; ?>"><strong><?=$messages?></strong></a>
                                </span>
                        </div>
                        <div class="input-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="truck" placeholder="Enter Mobile No." required  class="input-group__input form-control"/>
                        </div>
